...
function run_single_experiment(params::Dict)
    ...
    # Спектральный анализ для определения периода
...
#
# Зададим классические параметры из литературы.
...
# Расчет стационарных точек для базового эксперимента
...
# * β — эффективность охоты хищников
# * γ — смертность хищников
# * δ — эффективность конверсии
...
for (i, params) in enumerate(all_params)
    ...
    # Сохраняем полные данные для визуализации (только для некоторых комбинаций)
...
# ## Анализ влияния параметров на стационарные точки

println("\n" * "-"^60)
println("АНАЛИЗ ВЛИЯНИЯ ПАРАМЕТРОВ НА СТАЦИОНАРНЫЕ ТОЧКИ")
println("-"^60)

# Стационарная точка жертв: x* = γ/δ
...
# Стационарная точка хищников: y* = α/β
...
# ## Анализ влияния параметров на период колебаний

println("\n" * "-"^60)
println("АНАЛИЗ ВЛИЯНИЯ ПАРАМЕТРОВ НА ПЕРИОД КОЛЕБАНИЙ")
println("-"^60)

# Теоретическая зависимость: T = 2π/√(αγ)
...
# ## Влияние α (рождаемости жертв) на динамику
...
# ## Влияние γ (смертности хищников) на динамику
...
# Добавляем теоретическую кривую для γ=0.3
...
# Добавляем теоретическую кривую для α=0.1
...
# ##  Зависимость амплитуды колебаний от параметров
...
# ##  Сравнение фазовых портретов для разных параметров
...
# ## Тепловая карта: зависимость периода от α и γ
...
#
# Исследуем, как меняется характер колебаний при изменении параметров.

println("\n" * "-"^60)
println("АНАЛИЗ УСТОЙЧИВОСТИ СИСТЕМЫ")
println("-"^60)

# Коэффициент вариации как мера относительной амплитуды колебаний
...
# Находим наиболее и наименее устойчивые режимы
